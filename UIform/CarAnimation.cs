...
namespace UIform
{
    public partial class Form1 : Form
      { 
        carbo s = new carbo(); //BO cls er obj create 
        Car v = new Car(); //Entity cls er obj create
        enum position{
        ...
        private int objId; //Entity te use holo
        public Form1()
        {
            InitializeComponent();
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            carpanel.Invalidate();
            if (objpos == position.Right && x < 900)
            {
                x = s.getCarControltoRight(x, v.speed);
                objId = 0;
                // carpanel.Invalidate();
            }
            if (objpos == position.left && x > 130)
            {
                x = s.getCarControltoLeft(x, v.speed);
                objId = 1;
                carpanel.Invalidate();
            }
            if (objpos == position.Up && y>70)
            {
